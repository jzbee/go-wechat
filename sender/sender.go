...
func (this *CSender) CustomMessage(request *common.CPreviewMessageRequest, timeoutMS int64) (*common.CPreviewMessageResponse, error) {
	b, err := json.Marshal(request)
	if err != nil {
		return nil, err
	}
	method := http.MethodPost
	resBody, err := communicate.SendRequestWithToken(this.m_token, timeoutMS, &CustomMessageUrl, &method, nil, nil, b)
	if err != nil {
		return nil, err
	}
	response := common.CPreviewMessageResponse{}
